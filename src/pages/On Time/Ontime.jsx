...
const Dasboard = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white space-y-6 p-2 sm:p-4 md:p-6 custom-scrollbar min-h-0">

      {/* ── Top Premium Cards Insights Section (Completely clean typographic layout - NO Icons) ── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-1">
        
        {/* Total Orders Card */}
        <div className="bg-gradient-to-br from-amber-50/70 via-amber-50 to-amber-100/40 p-5 rounded-2xl border border-amber-200/60 shadow-sm flex flex-col justify-between h-28 transition-all hover:shadow-md duration-200">
          ...
          <div className="text-3xl font-black tracking-tight text-slate-900 font-mono my-1">
            {kpiStats.total}
          </div>
        ...
        {/* On Time Rate Card */}
        <div className="bg-gradient-to-br from-emerald-50/70 via-emerald-50 to-emerald-100/40 p-5 rounded-2xl border border-emerald-200/60 shadow-sm flex flex-col justify-between h-28 transition-all hover:shadow-md duration-200">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-extrabold text-slate-600 uppercase tracking-widest">
              On-Time Sales
            </span>
            <span className="text-[9px] font-black text-emerald-700 bg-emerald-100/60 px-2.5 py-0.5 rounded-full border border-emerald-200/40 font-mono">
              Sales Rate
            </span>
          </div>
          <div className="text-3xl font-black tracking-tight text-slate-900 font-mono my-1">
        ...
        {/* Urgent Orders Card */}
        <div className="bg-gradient-to-br from-rose-50/70 via-rose-50 to-rose-100/40 p-5 rounded-2xl border border-rose-200/60 shadow-sm flex flex-col justify-between h-28 transition-all hover:shadow-md duration-200">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-extrabold text-slate-600 uppercase tracking-widest">
              Urgent Focus
            </span>
        ...
        <div className="bg-gradient-to-br from-sky-50/70 via-sky-50 to-sky-100/40 p-5 rounded-2xl border border-sky-200/60 shadow-sm flex flex-col justify-between h-28 transition-all hover:shadow-md duration-200">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-extrabold text-slate-600 uppercase tracking-widest">
              Active Team
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-visible">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-visible">
          <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-2xl">
            <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">On Time Delivery Filters</h3>
            <button
              onClick={handleClearFilters}
              className="px-3 py-1 text-[10px] font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg transition-all active:scale-95"
          ...
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 overflow-visible">
            {/* Order Number Search */}
            <div className="space-y-1">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Order Number</label>
              <input 
                type="text" 
                placeholder="e.g. JF-01" 
                value={filters.orderNumber}
                onChange={(e) => { setFilters({ ...filters, orderNumber: e.target.value }); setCurrentPage(1); }}
                className="w-full h-[34px] px-3 bg-white border border-gray-350 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 rounded-lg text-xs outline-none transition-all font-bold text-slate-800" 
              />
            </div>

            {/* Client Name Search */}
            <div className="space-y-1">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Client Name</label>
              <input 
                type="text" 
                placeholder="Search client..." 
                value={filters.clientName}
                onChange={(e) => { setFilters({ ...filters, clientName: e.target.value }); setCurrentPage(1); }}
                className="w-full h-[34px] px-3 bg-white border border-gray-355 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 rounded-lg text-xs outline-none transition-all font-bold text-slate-800" 
              />
            </div>

            {/* Karigar Status Dropdown */}
            <div className="space-y-1 overflow-visible">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Karigar Status</label>
              <SearchableDropdown
            ...
            {/* Order Type Dropdown */}
            <div className="space-y-1 overflow-visible">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Order Type</label>
              <SearchableDropdown
            ...
            <div className="space-y-1 overflow-visible">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Order Stage</label>
              <SearchableDropdown
            ...
            <div className="space-y-1 overflow-visible">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Karigar Name</label>
              <SearchableDropdown
            ...
            {/* Late Status */}
            <div className="space-y-1 overflow-visible">
              <label className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider block">Late Status</label>
              <SearchableDropdown
                options={[
            ...
            <div className="space-y-2 bg-slate-50/50 p-3 border border-slate-200 rounded-xl">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-extrabold text-slate-700 uppercase tracking-wider">Sales Range Bounds</span>
                <span className="text-[10px] font-black text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full font-mono">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <input 
                    ...
                    className="w-full accent-amber-600 bg-slate-200 h-1 rounded-lg appearance-none cursor-pointer"
                  />
                  <input 
                    type="number" 
                    value={filters.leftDaysFrom}
                    onChange={(e) => { setFilters({ ...filters, leftDaysFrom: Number(e.target.value) }); setCurrentPage(1); }}
                    className="w-16 h-[26px] px-1 bg-white border border-gray-300 rounded-md text-[10px] text-center font-black text-slate-750 outline-none focus:border-amber-400 no-spinners" 
                  />
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col h-full min-h-[300px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col min-h-[300px]">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-1.5 h-3.5 bg-amber-500 rounded-full" />
            ...
            <table className="hidden sm:table w-full text-center border-collapse">
              <thead>
                <tr className="bg-slate-50/40 text-slate-500 border-b border-slate-100">
                  <th className="px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-center border-r border-slate-100/40">Rank</th>
                  <th className="px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-left border-r border-slate-100/40">Karigar Name</th>
                  ...
                  return (
                    <tr key={row.name} className="hover:bg-amber-50/20 transition-all group">
                      ...
                      <td className="px-3 py-1 border-r border-slate-100/50">
                        {isTopThree ? (
                          <span className="text-sm" title={`Rank ${idx + 1}`}>{rankIcons[idx]}</span>
                        ) : (
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/50">
          ...
          <div className="flex items-center gap-3 justify-between sm:justify-end">
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200/50 rounded-lg px-3.5 py-1.5 shrink-0 font-mono">
              <span className="text-[9px] font-black text-amber-700 leading-none">{deliveryData.length.toLocaleString()} Entries found</span>
            </div>
              ...
              {paginatedDeliveryData.map((row, idx) => (
                <tr 
                  ...
                  className={`hover:bg-amber-50/20 transition-colors group ${
                    idx % 2 === 0 ? "bg-white" : "bg-slate-50/15"
                  }`}
                >
                  {/* Sticky Order Number Column */}
                  <td className="px-4 py-2.5 text-xs font-black text-slate-900 sticky left-0 bg-inherit group-hover:bg-amber-50/40 z-10 shadow-[1px_0_0_#e2e8f0] border-r border-slate-200/80 text-center whitespace-nowrap font-mono">
                    {row.orderNo}
                  </td>
